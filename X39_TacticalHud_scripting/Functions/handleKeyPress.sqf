 ...
 */
 ...
 {
	scopeName "out";
	_currentKey = X39_TacticalHud_var_Keys select _i select 0;
	if(_currentKey select 0) then
	{
		_keys = actionKeys (_currentKey select 5);
		{
			...
			if(_this select 1 == _x) then {
				_this call (X39_TacticalHud_var_Keys select _i select 1);
				_handled = true;
				breakOut "out2";
			};
		}forEach _keys;
	...
	//_scriptToCall = X39_TacticalHud_var_Keys select _i select 1;
	_i = _i + 1;
 }count X39_TacticalHud_var_Keys;
 _handled